...
use super::{PositionsAttached, LogMessages, OutputMessages, MessageChannels, SelectedQueue, MessageInput};

pub fn positions_not_attached(attached: Res<PositionsAttached>) -> bool {
    !attached.0
}

pub fn orchestrator_not_spawned(query: Query<&Orchestrator>) -> bool {
    query.is_empty()
}

pub fn balancer_not_spawned(query: Query<&Balancer>) -> bool {
    query.is_empty()
}
